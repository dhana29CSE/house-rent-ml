...
st.markdown("📋 *Enter the house details below to estimate the monthly rent:*")

# --- Input Form ---
col1, col2 = st.columns(2)

with col1:
    area = st.number_input("📐 Area (sq.ft)", min_value=100, step=10, value=250)
...
with col2:
    ...
    year_built = st.number_input("🏗️ Year Built", min_value=1900, max_value=2025, value=2000)
    location = st.selectbox("📍 Location", ["Downtown", "Suburban", "Rural", "Town"])
    condition = st.selectbox("🏚️ Condition", ["Excellent", "Good", "Fair", "Poor"])
...
st.markdown("---")
st.markdown(
    "<center><small style='color:gray;'>Made with ❤️ by Dhana Lakshmi | Powered by Streamlit & Scikit-learn</small></center>",
    unsafe_allow_html=True
)